...
// PoC for a CONFIRMED consensus bug:
//
// For a NON-CEASING sidechain, ConnectBlock asserts that every certificate in the
// block is "top quality" for its scId (main.cpp:3762-3764):
//
//     bool isBlockTopQualityCert = highQualityCertData.count(cert.GetHash()) != 0;
//     if (sidechain.isNonCeasing())
//         assert(isBlockTopQualityCert);
//
// HighQualityCertData() (main.cpp:980-1009) reverse-iterates block.vcert and records
// only ONE cert per scId (the last in block order) as top quality. CheckCertificatesOrdering()
// (main.cpp:1047-1079) was relaxed to ALLOW multiple certs for one non-ceasing scId at
// increasing epochs in a single block. The "<=1 cert per non-ceasing scId per block" rule
// is enforced ONLY at mempool admission (main.cpp:1321), NOT in block validation.
//
// Therefore a block carrying TWO certs for the same non-ceasing scId (epoch N first,
// epoch N+1 second) makes the epoch-N cert non-top -> the process aborts via assert(false)
// during ConnectBlock. The epoch-N cert is processed first (forward order) and must pass
// IsCertApplicableToState (main.cpp:3720) to reach the assert; the epoch-N+1 cert never gets
// processed because the node aborts on the first one.
TEST_F(SidechainsConnectCertsBlockTestSuite, ConnectBlock_MultipleCerts_NonCeasing_SameScId_TriggersAssert)
{
    // Make the cumulative-tree map deterministic for the non-ceasing applicability path:
    // CheckEndEpochCumScTxCommTreeRoot() (coins.cpp:1247-1285) resolves referencedHeight by
    // looking up the cert's endEpochCumScTxCommTreeRoot in mapCumtreeHeight. UnloadBlockIndex()
    // does NOT clear mapCumtreeHeight, so stale entries from prior tests could point to a height
    // no longer present in chainActive. Clearing it here forces ExtendChainActiveToHeight() below
    // to repopulate it with heights that exist in the freshly-built chainActive.
    mapCumtreeHeight.clear();

    // setup NON-CEASING sidechain initial state...
    // A sidechain is non-ceasing iff version==2 AND withdrawalEpochLength==0
    // (sidechain.h:220-225 isNonCeasingSidechain). Non-ceasing SCs are active on REGTEST at
    // height >= 480 (fork10_nonceasingsidechainfork.cpp), so we keep all heights above that.
    CSidechain initialScState;
    uint256 scId = uint256S("aaaa");
    ...
    initialScState.lastTopQualityCertBwtAmount = 50;
    initialScState.balance = CAmount(100);
    // For non-ceasing SCs, CheckCertTiming (sidechain.cpp:158-206) requires:
    //   - referencedHeight >= lastInclusionHeight
    //   - inclusionHeight (== view.GetHeight()+1) in [lastInclusionHeight, INT_MAX]
    // view.GetHeight() resolves to 0 here (bestBlock is the dummy hash, not in mapBlockIndex),
    // so inclusionHeight == 1. Setting lastInclusionHeight = 0 satisfies both constraints for
    // any referencedHeight produced by ExtendChainActiveToHeight.
    initialScState.lastInclusionHeight = 0;
    initialScState.InitScFees();
    ASSERT_TRUE(initialScState.isNonCeasing());
    storeSidechain(scId, initialScState);

    // NOTE: non-ceasing sidechains never cease, so we do NOT add a ceasing event
    // (GetScheduledCeasingHeight() returns INT_MAX for non-ceasing - sidechain.cpp:278-281).

    // set relevant heights. For non-ceasing, the only acceptable epoch is
    // lastTopQualityCertReferencedEpoch + 1 (sidechain.cpp:167-173).
    int certEpochNumber = initialScState.lastTopQualityCertReferencedEpoch + 1; // 1988 (next expected)
    int certBlockHeight = 600; // > 480 (non-ceasing active) and > COINBASE_MATURITY

    // create coinbases to finance both certificate submissions (just in view)
    ...
    // extend blockchain to right height (also repopulates mapCumtreeHeight for SC-support blocks)
    chainSettingUtils::ExtendChainActiveToHeight(certBlockHeight - 1);

    // FIRST cert in block order: references epoch N (== lastTop+1). This is the cert that
    // becomes NON-top (HighQualityCertData keeps only the LAST cert per scId), yet it is the
    // FIRST processed in ConnectBlock's forward loop -> it triggers the assert.
    // It MUST satisfy IsCertApplicableToState:
    //   - CheckCertTiming: epoch == lastTop+1 (1988), referencedHeight (>=0) >= lastInclusionHeight (0) OK
    //   - CheckQuality: epoch (1988) != lastTop (1987) so the same-epoch quality rule does not apply OK
    //   - balance: epoch != lastTop -> scBalance == balance (100); BWT (90) <= 100 OK
    //   - cum tree root: SAMPLE_FIELD is present in mapCumtreeHeight and matches chainActive OK
    ...
    certEpochN.epochNumber = certEpochNumber;        // 1988 == lastTop + 1
    ...
    certEpochN.forwardTransferScFee = 0;
    certEpochN.mainchainBackwardTransferRequestScFee = 0;

    // SECOND cert in block order: references epoch N+1 (== lastTop+2). Because it is later in
    // block order, HighQualityCertData() records THIS cert as the (only) top-quality cert for
    // scId and skips the epoch-N cert above. CheckCertificatesOrdering() permits this (epochs
    // are strictly increasing in block order: 1988 then 1989).
    ...
    certEpochN1.epochNumber = certEpochNumber + 1;   // 1989 == lastTop + 2
    ...
    certEpochN1.forwardTransferScFee = 0;
    certEpochN1.mainchainBackwardTransferRequestScFee = 0;
    ...
    // add checkpoint to skip expensive checks (disables script/proof verification of inputs)
    CreateCheckpointAfter(certBlockIndex);

    // test: the epoch-N cert is non-top for a non-ceasing SC -> assert(isBlockTopQualityCert)
    // fires in ConnectBlock (main.cpp:3764), aborting the process. Note: scRelatedChecks is OFF
    // (matching the other ConnectBlock tests) but IsCertApplicableToState at main.cpp:3720 runs
    // UNCONDITIONALLY, so the epoch-N cert still traverses the applicability path before the assert.
    ASSERT_DEATH({
