...
mod tests {
    ...
    /// SECURITY-AUDIT PoC (sign-before-verify / WAL-523 class).
    ///
    /// A storage node signs an availability `StorageConfirmation` based only on key-presence
    /// (`is_stored_at_all_shards`); it never re-hashes the stored sliver bytes against the blob's
    /// Merkle commitment. Slivers that enter durable storage via the unverified shard-sync writer
    /// (`ShardStorage::put_sliver`, the `//TODO(WAL-523)` path) therefore (a) satisfy the
    /// confirmation gate and get signed for, while (b) failing client-side verification.
    ///
    /// This test emulates a malicious/garbage sync source by overwriting one shard's stored
    /// sliver for blob B1 with a sliver from a DIFFERENT blob B2, then shows the node still
    /// returns a signed confirmation for B1 even though the stored bytes do not verify against
    /// B1's metadata.
    #[tokio::test]
    async fn poc_signs_confirmation_for_unverifiable_sliver() -> TestResult {
        // Node 0 owns shards 0..=3; blob B1 (= BLOB) is fully stored (valid) and registered.
        let (cluster, _, blob) =
            cluster_with_partially_stored_blob(&[&[0, 1, 2, 3]], BLOB, |_, _| true).await?;
        let node = cluster.nodes[0].storage_node.clone();
        let config = node.inner.encoding_config.as_ref().clone();
        let blob_id = *blob.blob_id();

        // Baseline: with valid slivers, the node legitimately signs a confirmation.
        assert!(
            node.compute_storage_confirmation(&blob_id, &BlobPersistenceType::Permanent)
                .await
                .is_ok(),
            "baseline: node should sign a confirmation when slivers are valid",
        );

        // Emulate a malicious shard-sync source / on-disk bit-rot: take B1's own valid shard-0
        // primary sliver and flip a single byte. Same length, symbol size, and index (so it passes
        // the structural checks) but a different Merkle root (so it fails the content/hash check).
        // This is exactly the shape of corruption the unverified sync writer (`put_sliver`,
        // `//TODO(WAL-523)`) accepts without checking.
        let valid_primary = blob.assigned_sliver_pair(ShardIndex(0)).primary.clone();
        let mut bad_primary = valid_primary.clone();
        bad_primary.symbols.data_mut()[0] ^= 0xff;
        ...
        let shard0 = node
            ...
            .shard_storage(ShardIndex(0))
            .await
            .expect("node owns shard 0");
        shard0
            .put_sliver(blob_id, Sliver::Primary(bad_primary.clone()))
            .await?;

        // HARM 1: the node STILL signs an availability confirmation for the blob, even though
        // shard 0's stored primary sliver is now corrupt.
        assert!(
            node.compute_storage_confirmation(&blob_id, &BlobPersistenceType::Permanent)
                .await
                .is_ok(),
            "HARM: node signed an availability confirmation for unverifiable data",
        );

        // HARM 2: the stored (attested) sliver FAILS Merkle verification against the blob's
        // metadata, while the legitimate sliver verifies (control). The signed confirmation is
        // therefore unbacked by valid, serveable data.
        let meta = blob.metadata.metadata();
        assert!(
            bad_primary.verify(&config, meta).is_err(),
            "stored/attested sliver must FAIL Merkle verification",
        );
        assert!(
            valid_primary.verify(&config, meta).is_ok(),
            "control: the legitimate B1 sliver verifies",
        );
